...
const Header: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!recaptchaToken) {
      setSubmitStatus({ success: false, message: "Please verify you're not a robot" });
      return;
    }
    ...
    if (!isValidIndianMobile(formData.mobile)) {
      setSubmitStatus({ success: false, message: "Please enter a valid 10-digit Indian mobile number (starting with 6-9)" });
      return;
    }
                ...
                <div>
                  ...
                  {submitStatus?.success ? (
                    ...
                    <form onSubmit={handleSubmit}>
                      ...
                      {submitStatus && (
                        <div className={`mt-4 p-3 rounded-lg ${submitStatus.success ? "bg-green-900/50 text-green-300" : "bg-red-900/50 text-red-300"}`}>
                          {submitStatus.message}
                        </div>
                      )}
                      ...
                      <button
                        ...
                        className={`w-full mt-6 relative overflow-hidden px-6 py-3 bg-gradient-to-r from-blue-500 to-blue-600 text-white font-medium rounded-lg hover:from-blue-600 hover:to-blue-700 transition-all duration-300 group shadow-lg ${isSubmitting ? "opacity-70 cursor-not-allowed" : ""}`}
                      >
                        {isSubmitting ? (
